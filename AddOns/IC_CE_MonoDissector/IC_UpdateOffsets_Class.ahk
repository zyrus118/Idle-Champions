...
#Include, json.ahk
#Include, classExceptionHandler.ahk
...
class UpdateMemoryStructures
{
    ...
    structurePath := "%A_LineFile%\..\..\..\SharedFunctions\MemoryRead2\Structures\"
    ...
    __new(platform)
    {
        ...
        {
            this.isSteam := true
            this.exportFileName .= ".STEAM"
        }
        else if (platform == "EGS")
        {
            this.isSteam := false
            this.exportFileName .= ".EGS"
        }
        else
            ExceptionHandler.ThrowFatalError("Parameter string must be 'Steam' or 'EGS'.`nInvalid Parameter: " . platform, -2)

        file := FileOpen(this.exportFileName, "r")
        if !file
                ExceptionHandler.ThrowFatalError("Could not find file.`nfile: " . this.exportFileName, -2)
    ...
    UpdateStructureFiles2()
    {
        ...
        loop, %count%
        {
            fullFilePath := this.structurePath . this.structureFiles[A_Index]
            ...
            while currentPos
            {
                ...
                if !classNamePos
                {
                    ...
                    structString .= ";Failed to match a classname. currentPos: " . currentPos
                    continue
                }
                if !(this.data.classes.HasKey(className))
                {
                    ...
                    structString .= ";Failed to find class name in data file. Class Name: " . className
                    continue
                }
