...
function miCreateNewOffsetFileClick(sender)
  local open = createOpenDialog(owner)
  open.Execute()
  local file = open.FileName
  local handle = io.open(file)
  io.input(handle)
  local str = io.read("a")
  --print(str, 'first string print')
  --string.gsub(str,';FB-.+;FE', parseFields(string.gmatch(str,';FB-.+;FE')))
  for fieldList in string.gmatch(str, ";FB-.-;FE") do
    string.gsub(str, fieldList, parseFields(fieldList))
  end
  print("end")
end

function parseFields(str)
  --print(str, "End Param passed to parseFields")
  local className = string.match(str, "%S+", 5)
  --print("Class Name: ", className)
  local priority=nil  
  --first enum all images
  local class=getClass(className)
  print("class name passed to func: " .. className)
  print("Returned namespace: " .. class.namespace .. "  class name: " .. class.classname .. "  class fq name: " .. class.fqname)
  --if (monopipe==nil)  then
  --  LaunchMonoDataCollector()
  --end
  local fields = mono_class_enumFields(class)
  if fields then
    for i=1, #fields do
      print("OS (int): " .. fields[i].offset .. "   Field Name: " .. fields[i].name .. "   Type: " .. fields[i].typename .. "  isStatic: " .. fields[i].isStatic)
    end
  else
    print("parseFields func failed to define fields")
  end
  print("fields test")
  return str
end
...
function getClass(className)
  ...
  if classes~=nil then
    for i=1, #classes do
      ...
      if string.find(classes[i].fqname, className) then
        return classes[i]
      end
    end
  else
  ...
  print("getClass failed to find " .. className)
end
...
function mono_setMonoMenuItem(usesmono, usesdotnet)
  ...
  if usesmono or usesdotnet then
    ...
    if (miMonoTopMenuItem==nil) then
      ...
      if mfm then
        ...
        mi=createMenuItem(miMonoTopMenuItem)
        mi.Caption=translate("Create new offset file")
        --mi.Shortcut="Ctrl+Alt+T"
        mi.OnClick=miCreateNewOffsetFileClick
        mi.Name='miCreateNewOffsetFile'
        miMonoTopMenuItem.Add(mi)
    ...
    miMonoTopMenuItem.miCreateNewOffsetFile.Visible=true
    ...
    elseif usesdotnet and not usesmono then  
      ...
      miMonoTopMenuItem.miCreateNewOffsetFile.Visible=false 
  ...
  if (not usesmono) and (not usesdotnet) then  
    ...
    if miMonoTopMenuItem~=nil then
      ...
      miMonoTopMenuItem.miCreateNewOffsetFile.destroy() --clean up the onclick handler
